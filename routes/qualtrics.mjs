...
async function exportSurvey(apiToken, surveyId, dataCenter, fileFormat) {
    console.log("Retrieving survey response ...")
    ...
    try {
        ...
        const outputDir = path.join(__dirname, 'MultiAgent');
...
const cleanCSV = async (inputFile, outputFile, responseIdToKeep) => {
    console.log("in cleanCSV")
    console.log(fs.readdirSync(path.join(__dirname, 'MultiAgent')));
...
// For knowledge -- 1 is true, 2 is false, 3 is unsure
    ...
    return new Promise((resolve, reject) => {
        ...
        fs.createReadStream(inputFile)
            ...
            .on('data', (row) => {
                ...
                knowledge = [...wrong, ...unsure];
            })
            .on('end', () => {
                resolve({ correct, knowledge, attitudes });
            })
            .on('error', reject);
    });
};
...
router.get('/getSurveyResponses', async (req, res) => {
    try {
        console.log("In server, getting survey responses...")
        ...
        const surveyId = req.query.surveyId || "SV_0CXbC5QOEHHOcUm";
...
router.post('/processSurveyResponses', async (req, res) => {
    ...
    try {
        ...
        const inputFile = path.join(__dirname, 'MultiAgent', 'MultiAgent 2 - Pre.csv');
        const outputFile = path.join(__dirname, 'MultiAgent', 'MultiAgent 2_Cleaned.csv');
...
router.post('/scoreSurveyResponses', async (req, res) => { 
    ...
    try {
        ...
        const inputFile = path.join(__dirname, "MultiAgent/MultiAgent 2_Cleaned.csv");
        ...
        sql.connect(config, function (err) {
            ...
            const queryString = `UPDATE CTStudy SET topics = @userProfileJSON WHERE id = @id`;
        
            request.input('id', sql.NVarChar, id);
            request.input('userProfileJSON', sql.NVarChar, userProfileJSON);
            ...
            request.query(queryString, function (err, recordset) {
              ...
              res.json({ message: "Responses have been scored, stored in database, and relevance scores have been assigned!" });
            });
          });
    } catch (error) {
